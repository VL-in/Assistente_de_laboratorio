...
| Índice | Indexação RAG | `rag/index_txtai.py` + TEI — modelo [`intfloat/multilingual-e5-small`](https://huggingface.co/intfloat/multilingual-e5-small) (até 512 tokens) |
| Busca | Teste RAG / Chat | Similaridade semântica; trechos citam projeto e arquivo |

**Primeira indexação:** na aba **Indexação RAG**, use **Substituir índice existente** (recomendado). Na primeira subida do Compose, o contêiner `embeddings` baixa o modelo TEI (~470 MB); aguarde o healthcheck ficar saudável antes de indexar.
