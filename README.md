...
Aplicação web **MVP offline-first** para apoiar laboratórios de P&D. O assistente responde perguntas sobre documentos internos (**RAG**), analisa planilhas de ensaios (**OLAP**) e faz predições de afinidade anticorpo–antígeno (**ML tradicional**). Toda a infraestrutura roda localmente via Docker; apenas as chamadas de linguagem vão para a API do OpenRouter.

| Documento | Função |
|-----------|--------|
| [`.env.docker.example`](.env.docker.example) | Modelo de variáveis de ambiente — copiar para `.env` na raiz |
...
├── .env.docker.example             # Modelo de variáveis (segredos ficam só no .env)
├── scripts/
...
**Convenção de projeto:** cada **subdiretório imediato** da pasta raiz configurada (ex.: `Projetos/`) é um projeto (`project_id` = nome da pasta). Subpastas internas como `planning/` ou `results/` pertencem ao mesmo projeto.
...
> **Por que OpenRouter?** Ele expõe centenas de modelos (incluindo os gratuitos) através de uma única API compatível com OpenAI. O custo de modelos pagos aparece no seu painel; os modelos `:free` são gratuitos com limites de requisição.
...
|----------|-------------|-----------------|
| `PROJETOS_HOST_DIR` | Sim | Caminho absoluto no host da pasta de projetos (ex.: `D:/Vanessa/AI_project/Projetos`) |
| `OPENROUTER_API_KEY` | Sim | Chave em https://openrouter.ai/settings/keys (começa com `sk-or-v1-...`) |
| `LLM_MODEL` | Não | Slug do modelo (padrão `openrouter/auto`) — veja [Escolha do modelo](#escolha-do-modelo) |
| `LLM_BASE_URL` | Não | Padrão `https://openrouter.ai/api/v1` |
| `OPENROUTER_APP_TITLE` | Não | Nome exibido nos rankings do OpenRouter |
| `STREAMLIT_PORT` | Não | Padrão `8502` |
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` | Não | Observabilidade via Langfuse — veja [Observabilidade](#observabilidade-langfuse) |
...
2. **Documentos** — informe a raiz (ou use a do `.env`) e clique em **Escanear pastas**.
...
A imagem verifica `http://127.0.0.1:8502/_stcore/health`:
...
2. Vá em **Settings → Keys** e clique em **Create Key**.
3. Cole o valor em `OPENROUTER_API_KEY` no `.env`.
...
`LLM_MODEL` aceita qualquer slug de https://openrouter.ai/models:
...
> Os perfis de sampling em `qwen35_inference.py` aplicam parâmetros validados do Qwen3.5 apenas quando o slug contém `qwen3.5`. Outros modelos recebem um perfil neutro.
...
| `LLM_TIMEOUT_S=120` | Timeout das chamadas ao OpenRouter (segundos) |
...
O `eval_bootstrap.py` aplica esses valores automaticamente. Só é necessário sobrescrever se quiser valores diferentes.
