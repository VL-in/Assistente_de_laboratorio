...
class ParseRouterJsonTests(unittest.TestCase):
    def test_plain_json(self) -> None:
        self.assertEqual(_parse_router_json('{"documents": true, "spreadsheets": false}'), (True, False))

    def test_with_markdown_fence(self) -> None:
        raw = '```json\n{"documents": false, "spreadsheets": false}\n```'
        self.assertEqual(_parse_router_json(raw), (False, False))
...
class RuleFallbackTests(unittest.TestCase):
    def test_social(self) -> None:
        ...
        self.assertEqual((d.use_documents, d.use_spreadsheets), (False, False))
...
class ClassifyChatRoutesTests(unittest.TestCase):
    ...
    def test_llm_path(self, mock_llm: MagicMock, _enabled: MagicMock) -> None:
        mock_llm.return_value = ChatRouteDecision(True, False, "llm")
        d = classify_chat_routes(
